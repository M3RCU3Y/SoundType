...
| `SoundType-EnterDing` | SoundType Enter Ding | CC0/public-domain Freesound and OpenGameArt bell/chime recordings listed in `SOURCE.txt` | Processed CC0 source recordings. Converted to mono 44.1 kHz WAV, trimmed, filtered, mixed, normalized with limiter headroom, and faded for Enter overlays. The classic bell and typewriter-style variants keep longer residual ring tails; newer variants include `sd_0.wav`, `pleasing-bell.wav`, and `bell.wav`. | `pack.json`, `SOURCE.txt`, `THIRD_PARTY_NOTICES.md` |
